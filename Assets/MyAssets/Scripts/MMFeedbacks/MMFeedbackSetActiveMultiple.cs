...
namespace MoreMountains.Feedbacks
{
    ...
    /// </summary>
    [AddComponentMenu("")]
    [FeedbackHelp("This feedback allows you to change the state of the target gameobjects from active to inactive (or the opposite), on init, play, stop or reset. For each of these you can specify if you want to force a state (active or inactive), or toggle it (active becomes inactive, inactive becomes active).")]
    [FeedbackPath("GameObject/Set Active Multiple")]
    public class MMFeedbackSetActiveMultiple : MMFeedback
    {
        ...
        protected override void CustomInitialization(GameObject owner)
        {
            ...
            if (Active && (TargetGameObjectList != null))
            {
                if (SetStateOnInit)
                {
        ...
        {
            foreach (var TargetGameObject in TargetGameObjectList)
            {
                switch (state)
                {
