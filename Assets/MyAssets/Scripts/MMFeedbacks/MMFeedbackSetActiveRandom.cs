...
{
    /// <summary>
    /// Turns a random object from list active or inactive at the various stages of the feedback
    /// </summary>
    [AddComponentMenu("")]
    [FeedbackHelp("This feedback allows you to change the state of random gameobject from the list from active to inactive (or the opposite), on init, play, stop or reset. For each of these you can specify if you want to force a state (active or inactive), or toggle it (active becomes inactive, inactive becomes active).")]
    [FeedbackPath("GameObject/Set Active Random")]
    public class MMFeedbackSetActiveRandom : MMFeedback
    {
        ...
        protected override void CustomInitialization(GameObject owner)
        {
            ...
            if (Active && (RandomGameObjects != null))
            {
                if (SetStateOnInit)
                {
        ...
        {
            GameObject TargetGameObject = GetRandomObject();
        ...
        {
            int randIndex = Random.Range(0, RandomGameObjects.Count);
            return RandomGameObjects[randIndex];
        }
    }
}