...
namespace MyAssets.Scripts.PoseAnimancer.AnimancerJobs
{
    ...
    /// 
    public sealed class TranslateRoot : AnimancerJob<TranslateRoot.Job>, IDisposable
    {
        /************************************************************************************************************************/
        #region Initialisation
        /************************************************************************************************************************/

        public TranslateRoot(AnimancerPlayable animancer, Vector3 axis, NativeArray<TransformStreamHandle> bones)
        {
            ...
            _Job = new Job
            {
                ...
                bones = bones,
                axis = axis,
                distance = AnimancerUtilities.CreateNativeReference<float>(),
            };
        ...
        public struct Job : IAnimationJob
        {
            ...
            public void ProcessAnimation(AnimationStream stream)
            {
                ...
                var offset = worldAxis * distance[0];
                ...
                // root.SetPosition(stream, offset + root.GetPosition(stream));
                
                for (int i = bones.Length - 1; i >= 0; i--)
                {
                    var bone = bones[i];
                    // bone.SetPosition(stream, offset + bone.GetPosition(stream));
                    bone.SetPosition(stream, offset + bone.GetPosition(stream));
                }
            }
