...
Shader "Hidden/MicroSplat/StreamUpdate"
{
	...
	SubShader
	{
		...
		Pass
		{
         ...
         float _WetnessEvaporation;
         float _BurnEvaporation;
         ...
         float2 Sample(int x, int y, half height, float2 origUV, half4 curData, float2 dt, half2 collider)
         {
            float2 offset = float2(x,y);
            offset *= _TerrainHeight_TexelSize.xy;
            float2 uv = saturate(origUV + offset);
            float rawHeight = SampleInstancedTerrainDesc(uv);
            float2 nData = tex2D(_MainTex, uv).zw;
         ...
         half4 GetSpawners(float2 uv)
         {
            ...
            float4 curData = tex2D(_MainTex, i.uv);
            float height = SampleInstancedTerrainDesc(i.uv);
